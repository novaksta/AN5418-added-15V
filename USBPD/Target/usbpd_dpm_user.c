...
void USBPD_DPM_GetDataInfo(uint8_t PortNum, USBPD_CORE_DataInfoType_TypeDef DataId, uint8_t *Ptr, uint32_t *Size)
{
	...
	case USBPD_CORE_DATATYPE_SNK_PDO:           /*!< Handling of port Sink PDO, requested by get sink capa*/
		...
		*Size *= 4;
		...
		//    *Size = 4;
		//    (void)memcpy((uint8_t *)Ptr, (uint8_t *)&DPM_Port.DPM_RequestedVoltage, *Size);
		// break;
...
//		break;
	case USBPD_CORE_DATATYPE_RCV_SRC_PDO : // CubePkg G081 example inspired ()
		if (Size <= (USBPD_MAX_NB_PDO * 4))
		{
			DPM_Port.DPM_NumberOfRcvSRCPDO = (Size / 4);
			memcpy((uint8_t*)DPM_Port.DPM_ListOfRcvSRCPDO, Ptr, Size);
		}
		break;
...
void USBPD_DPM_SNK_EvaluateCapabilities(uint8_t PortNum, uint32_t *PtrRequestData, USBPD_CORE_PDO_Type_TypeDef *PtrPowerObjectType)
{
	...
	rdo.d32 = 0;
	/* Prepare the requested pdo -- take the default*/
	...
	for (uint32_t j = 0; j < size; ++j){//check all snk confs
		//Parse as e.g. AN5418 section 5.1.4
		uint32_t uSnk = snkpdolist[j] & 0xffc00;
		uint16_t iSnk = snkpdolist[j] & 0x3ff;
		for(int i = 0; i < DPM_Port.DPM_NumberOfRcvSRCPDO; ++i ){//check all src confs
			uint32_t uSrc = DPM_Port.DPM_ListOfRcvSRCPDO[i] & 0xffc00;
			uint16_t iSrc = DPM_Port.DPM_ListOfRcvSRCPDO[i] & 0x3ff;
			if((uSrc == uSnk) && (iSrc>= iSnk)){
				// Means: if suitable conf. with higher index take it
				// In application you may possibly evaluate watts by comparing u*i/whatever
				rdo.FixedVariableRDO.ObjectPosition = i + 1; // freaking-matlab-indexing
				rdo.FixedVariableRDO.OperatingCurrentIn10mAunits  = iSnk; // fill what Sink requests
				rdo.FixedVariableRDO.MaxOperatingCurrent10mAunits = iSrc; // no more than Src can give
			}
		}
	}
